...
const AnimalList = () => {
  ...
  useEffect(() => {
    if (!isLoading) {
      const animalDT = animalsData.map((animal, index) => {
        return { id: index, img: animal };
      });
      setAnimals(animalDT);
    }
  ...
  return (
    <>
      ...
      >
        <Box sx={{ flexGrow: 1 }} style={{ padding: "100px" }}>
          <p>
            Site build wtih Recoil to managment state of React, only is a demo
          </p>
          <Grid
            container
            spacing={{ xs: 3 }}
